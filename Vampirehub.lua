...
local function getgenv().AutoAbility()
        ...
        if value then
            Connections_Manager['Auto Parry'] = RunService.PreSimulation:Connect(function()
                ...
                for _, Ball in pairs(Balls) do
                    ...
                    if getgenv().getgenv().RandomParryAccuracyEnabled then
                        if Speed < 200 then
                ...
                if Ball_Target == tostring(Player) and Distance <= Parry_Accuracy and Phantom then
                    ...
                    if getgenv().getgenv().InfinityDetection and Infinity then
                        return
                    end

                    if Ball_Target == tostring(Player) and Distance <= Parry_Accuracy then
                        if getgenv().getgenv().AutoAbility and getgenv().AutoAbility() then
                            return
                        end
                    end

                    if Ball_Target == tostring(Player) and Distance <= Parry_Accuracy then
                        if getgenv().getgenv().CooldownProtection and cooldownProtection() then
                            return
                        end
                        ...
                        if getgenv().getgenv().AutoParryKeypress then
                            VirtualInputService:SendKeyEvent(true, Enum.KeyCode.F, false, nil)
                        else
                            Auto_Parry.Parry(Selected_Parry_Type)
                        end
...
module:AddToggle({
	...
	Callback = function(value)
        getgenv().getgenv().RandomParryAccuracyEnabled = value
    end
})
...
module:AddToggle({
	...
	Callback = function(value)
        getgenv().getgenv().InfinityDetection = value
    end
})
...
module:AddToggle({
	Name = "Keypress",
	Callback = function(value)
        getgenv().getgenv().AutoParryKeypress = value
    end
})
        ...
        if value then
            Connections_Manager['Auto Spam'] = RunService.PreSimulation:Connect(function()
                ...
                if Distance <= Spam_Accuracy and Parries > threshold then
                    if getgenv().getgenv().SpamParryKeypress then
                        VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.F, false, game) 
                    else
                        Auto_Parry.Parry(Selected_Parry_Type)
                    end
...
SpamParry:AddToggle({
    Name = "Keypress",
    Callback = function(value)
        getgenv().getgenv().SpamParryKeypress = value
    end
})
	...
	Callback = function(value)
        if value then
            Connections_Manager['Manual Spam'] = RunService.PreSimulation:Connect(function()
                ...
                if getgenv().getgenv().ManualSpamKeypress then
                    VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.F, false, game) 
                else
                    Auto_Parry.Parry(Selected_Parry_Type)
                end
...
ManualSpam:AddToggle({
    Name = "Keypress",
    Callback = function(value)
        getgenv().getgenv().ManualSpamKeypress = value
    end
})
        ...
        if value then
            Connections_Manager['Triggerbot'] = RunService.PreSimulation:Connect(function()
                ...
                for _, Ball in pairs(Balls) do
                    ...
                    if getgenv().getgenv().TriggerbotInfinityDetection and Infinity then
                        return
                    end
    
                    if Ball_Target == tostring(Player) then
                        if getgenv().getgenv().TriggerbotKeypress then
                            VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.F, false, game) 
                        else
                            Auto_Parry.Parry(Selected_Parry_Type)
                        end
...
Triggerbot:AddToggle({
    Name = "Infinity Detection",
    Callback = function(value)
        getgenv().getgenv().TriggerbotInfinityDetection = value
    end
})
...
Triggerbot:AddToggle({
    Name = "Keypress",
    Callback = function(value)
        getgenv().getgenv().TriggerbotKeypress = value
    end
})
    ...
    Callback = function(value)
        if value then
            Connections_Manager['Lobby AP'] = RunService.Heartbeat:Connect(function()
                ...
                if getgenv().getgenv().LobbyAPRandomParryAccuracyEnabled then
                ...
                if Ball_Target == tostring(Player) and Distance <= LobbyAPParry_Accuracys then
                    if getgenv().getgenv().LobbyAPKeypress then
                        VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.F, false, game) 
                    else
                        Auto_Parry.Parry(Selected_Parry_Type)
                    end
...
LobbyAP:AddToggle({
    Name = "Random Parry Accuracy",
    Callback = function(value)
        getgenv().getgenv().LobbyAPRandomParryAccuracyEnabled = value
    end
})
...
LobbyAP:AddToggle({
    Name = "Keypress",
    Callback = function(value)
        getgenv().getgenv().LobbyAPKeypress = value
    end
})
...
local getgenv().Spinbot = player:AddSection({
	Name = "getgenv().Spinbot",
	Position = "right",
});

getgenv().Spinbot:AddToggle({
    Name = "getgenv().Spinbot",
    Callback = function(value)
        getgenv().getgenv().Spinbot = value
        if value then
            getgenv().spin = true
